...
Most AI-for-notes tools draft, summarize, and auto-file *for* you. vault-mind is the opposite
philosophy, and it's deliberate: **writing your notes is the thinking, and the thinking is the
...
skill reads. That's what makes the kit portable across differently-shaped vaults.
...
| `/vault-mind:recommend` | Reading suggestions grounded in what you've read/rated/noted + web search. | opt-in stubs |
| `/vault-mind:fetch` | Download the public source behind a reading's URL into your assets dir. | source files + 1 metadata key |
...
| `/vault-mind:feynman` | You explain a concept back; it finds the holes vs. your note + the literature. | no |
...
- **Assumes** an atomic-note vault with YAML frontmatter and `[[wikilinks]]`. Sparse or
  ...
  work with).
- **`quiz` spacing** uses a `reviewed`-role date it stamps itself — a minimal built-in SR. If
  you already run a dedicated spaced-repetition plugin, prefer that; this is for vaults without one.
